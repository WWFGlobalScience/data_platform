...
WDPA_gdb=r"L:\data_platform\_Data\Pilots_data\WDPA_Sep2024_Public\WDPA_Sep2024_Public.gdb"
WDPA_polys= f'{WDPA_gdb}\WDPA_poly_Sep2024'
WDPA_points= f'{WDPA_gdb}\WDPA_point_Sep2024'
...
arcpy.management.Dissolve('polys_in_scapes', 'polys_all_dis', multi_part=True) #polys_all_dis is the WDPA flat layer
...
# Or make a note in summary graphics that IUCN categories are not necessarily mutually exclusive-maybe we don't use a pie chart. 

# Dissolve on IUCN_CAT. to get only IUCN categories.
arcpy.management.Dissolve('polys_IUCN', 'polys_IUCN_dis', 'IUCN_CAT', multi_part=True) #polys_IUCN_dis is the IUCN flat layer
print("iucn polys dissolved")

#Erase IUCN layer to get ‘Other’ layer with no overlap
arcpy.analysis.Erase('polys_all_dis', 'polys_IUCN_dis', 'polys_other_erase') #polys_other_erase is the IUCN-other flat layer
...
if matchcount == 0:
    ...
    arcpy.analysis.Union(['pts_all_dis', 'polys_other_erase'], 'other_union')
    print("union done")
    ...
    # union both layers to combine with polys
    arcpy.analysis.Union(['pts_other_erase', 'polys_other_erase'], 'other_union') #union might not work bc need to preserve attributes. test out merge instead, watch out for overlap in output.
    arcpy.analysis.Union(['pts_buff_IUCN_dis', 'polys_IUCN_dis'], 'iucn_union')
    print("unions done")

    #summarize iucn_union and other_union within pilot scapes to calculate area in hectares for each category
    arcpy.analysis.SummarizeWithin(scapes, 'iucn_union', 'PS_IUCN_sum', keep_all_polygons=True, sum_shape=True, shape_unit='HECTARES', group_field='IUCN_CAT', add_group_percent=True, out_group_table='IUCN_sum')
    arcpy.analysis.SummarizeWithin(scapes, 'other_union', 'PS_oth_sum', keep_all_polygons=True, sum_shape=True, shape_unit='HECTARES', group_field='FID_pts_all_dis', add_group_percent=True, out_group_table='other_sum')
    print("summarize within pilot scapes done")
...
columns = ['Join_ID','sum_Area_HECTARES', 'Area_HA'] #
...
#combine tables
other=other.set_index('Join_ID')
newdf = pd.concat([area_pivot, pct_pivot, other], axis=1)

#reorder columns
col_order=['Ia_HA', 'Ib_HA', 'II_HA', 'III_HA', 'IV_HA', 'V_HA', 'VI_HA', 'Other_HA', 'Ia_pct',
...
# Join fields in summary tables (PS_oth_sum + IUCN_sum) to copy of scapes dataset
...
arcpy.management.AlterField('PS_IUCN_sum', 'sum_Area_HECTARES', 'PA_HA')
arcpy.management.JoinField(scapes, 'ScapeID', 'PS_IUCN_sum', 'JOIN_ID', fields=['PA_HA'])
arcpy.management.CalculateField(scapes, 'PA_pct', "!PA_HA!/!Area_HA!*100", 'PYTHON', '', 'FLOAT')
arcpy.management.CalculateField(scapes, 'NP_HA', "!Area_HA!-!PA_HA!", 'PYTHON', '', 'FLOAT')
arcpy.management.CalculateField(scapes, 'NP_pct', "!NP_HA!/!Area_HA!*100", 'PYTHON', '', 'FLOAT')
