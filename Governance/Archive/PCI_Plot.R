...
pci <- corrupt[,c(1:3,44:61)]

colnames(pci) <- c("Country", "State", "ISO", 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021)

pci <- pci %>% pivot_longer(cols = 4:21, names_to = "Year", values_to = "Value") %>%
...
pci_plot <- ggplot(pci[!is.na(pci$Value),], aes(x = Year, y = Value, group = State, color = Country)) +
  geom_line() +
  geom_point(
    aes(fill = Country),
    size = 2.5, alpha = 0.5, 
