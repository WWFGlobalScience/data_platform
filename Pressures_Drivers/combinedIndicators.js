// Pilot landscapes: Pantanal, TriDom, Diana seascape, Upper Amazon.
...
var countries = ee.FeatureCollection("USDOS/LSIB_SIMPLE/2017").filter(ee.Filter.inList('country_na', namesFilter));
...
// I suspect that we actually want these by hydrosheds per landscape, so... 
function addDamsAndWaterStressPerHydroshed(fc){
  ...
  return fc.map(function(f){
    return stress.filterBounds(f.geometry()).map(function(h){
      ...
      return ee.Feature(h.geometry()), h.select(['HYBAS_ID', 'bws_label', 'wdp_label', 'wsb_label'], ['basinID', 'baselineWaterStress', 'waterDepletion', 'blueWaterScarcity'])
...
//   'folder': [ INSERT YOUR FOLDER NAME HERE ],
