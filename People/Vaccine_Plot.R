...
vaccine <- people[,c(1:3,111:139)]

colnames(vaccine) <- c("Country", "State", "ISO", 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023)

vaccine <- vaccine %>% pivot_longer(cols = 4:32, names_to = "Year", values_to = "Value") %>%
...
vaccine_plot <- ggplot(vaccine[!is.na(vaccine$Value),], aes(x = Year, y = Value, group = State, color = Country)) +
  geom_line() +
  geom_point(
    aes(fill = Country),
    size = 2.5, alpha = 0.5, 
  ...
  scale_y_continuous(
    name = "Mean percent of children < 1yo\nwith BCG, DTP1, DTP2, DTP3, and Measles vaccines"
  ) +
