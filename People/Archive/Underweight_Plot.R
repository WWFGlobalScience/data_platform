...
people <- read_xlsx(path = "People_Indicators_2.xlsx", na = "NA")

countries <- c("Colombia")
country_cols <- c("#E69F00", "#56B4E9", "#009E73")

underweight <- people[,c(1:3,28:54)]

colnames(underweight) <- c("Country", "State", "ISO", 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021)

underweight <- underweight %>% pivot_longer(cols = 4:30, names_to = "Year", values_to = "Value") %>%
...
underweight_plot <- ggplot(underweight[!is.na(underweight$Value),], aes(x = Year, y = Value, group = State, color = Country)) +
  geom_line() +
  geom_point(
    aes(fill = Country),
    size = 2.5, alpha = 0.5, 
  ...
  scale_y_continuous(
    name = "Percent of severely underweight children < 5yo\n(3+ standard deviations below WHO median weight for age)"
  ) +
